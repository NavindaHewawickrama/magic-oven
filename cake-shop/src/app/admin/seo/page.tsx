...
function Field({ label, placeholder, textarea }: { label: string; placeholder: string; textarea?: boolean }) {
  return (
    <div>
      ...
      {textarea ? (
        <textarea rows={3} placeholder={placeholder} className="mt-1.5 w-full resize-none rounded-xl border border-line bg-cream px-4 py-2.5 text-sm outline-none focus:border-brick" />
      ) : (
        <input placeholder={placeholder} className="mt-1.5 w-full rounded-xl border border-line bg-cream px-4 py-2.5 text-sm outline-none focus:border-brick" />
      )}
    </div>
  );
}
    ...
    <DashboardShell title="SEO Settings" subtitle="Admin CMS" nav={adminNav} activeHref="/admin/seo">
      <div className="space-y-8">
        <div className="rounded-2xl border border-line bg-white/70 p-6">
          <p className="font-display text-lg font-semibold">Homepage metadata</p>
        ...
        <div className="rounded-2xl border border-line bg-white/70 p-6">
          <p className="font-display text-lg font-semibold">Open Graph & Twitter Card</p>
        ...
        <div className="rounded-2xl border border-line bg-white/70 p-6">
          <p className="font-display text-lg font-semibold">Indexing</p>
