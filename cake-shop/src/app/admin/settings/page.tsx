...
function Field({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <input defaultValue={value} className="mt-1.5 w-full rounded-xl border border-line bg-cream px-4 py-2.5 text-sm outline-none focus:border-brick" />
    </div>
  );
}
    ...
    <DashboardShell title="Settings" subtitle="Admin CMS" nav={adminNav} activeHref="/admin/settings">
      <div className="space-y-8">
        <div className="rounded-2xl border border-line bg-white/70 p-6">
          <p className="font-display text-lg font-semibold">Site logo</p>
          ...
          <p className="mt-2 text-xs text-espresso-soft">Used in the navbar, footer, and favicon. Recommended: square PNG or SVG, transparent background.</p>
        </div>

        <div className="rounded-2xl border border-line bg-white/70 p-6">
          <p className="font-display text-lg font-semibold">Business details</p>
